...
## `strat_kmeans` {.unnumbered}

We provide a preliminary example of the `strat_kmeans` algorithm to prepare our input data but here are a few more.

```{r,warning=F,message=F}
 #--- perform stratification using k-means ---#
 kmeans <- strat_kmeans(mraster = mraster, # input
...
The `strat_pcomp` algorithm calculates principal components on the input `mraster` then divides data into equal sized strata. Strata can be defined by using the primary (PC1) only, or primary & secondary (PC2) principal components. If `nStata2` is defined, PC2 is used and the total number of strata will be $$nStrata * nStrata2$$.
...
```
Once the breaks have been created we can input them into the function.

```{r,warning=F,message=F}
strat_breaks(mraster = mraster, # input
             metric = "zmax", # covariate of interest - can be numeric index of mraster or character string
...
```{r,warning=F,message=F}
strat_breaks(mraster = mraster, # input
             metric = 1, # primary covariate of interest - can be numeric index of mraster or character string
             metric2 = "zsd", # secondary covariate of interest - can be numeric index of mraster or character string
             breaks = br.max, # breaks for primary covariate
...
The `strat_quantiles` function stratifies data into the user-defined number of quantiles. A single `metric` can be defined or an additional `metric2` can be supplied. `nQuant` and `nQuant2` correspond to the desired number of quantiles for `metric` and metric2` respectively. If `nQuant` is defined, the total number of strata will be $$nStrata = nQuant * nQuant2$$

```{r,warning=F,message=F}
strat_quantiles(mraster = mraster, # input
			          metric = 4, # primary covariate of interest - can be numeric index of mraster or character string
...
```{r,warning=F,message=F} 
strat_quantiles(mraster = mraster, # input
			          metric = "zsd", # primary covariate of interest - can be numeric index of mraster or character string
			          metric2 = "zq95", # secondary covariate of interest - can be numeric index of mraster or character string
...
```{r,warning=F,message=F, eval = FALSE} 
strat_osb(mraster = mraster, # input
	        metric = 4, # covariate of interest - can be numeric index of mraster or character string
